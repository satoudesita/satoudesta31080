...
@st.cache
def load_data():
    return pd.read_excel("yusyutu.xlsx")
...
def prepare_data():
    labels = words_df['順位'].tolist()[:4]  # 1位から4位のラベルを取得
    sizes = words_df[['割合1', '割合2', '割合3', '割合4']].iloc[np.random.randint(len(words_df))].tolist()
    return labels, sizes
...
# 円グラフの描画
st.write("円グラフを生成するには下のボタンをクリックしてください。")

if st.button('円グラフを生成'):
    labels, sizes = prepare_data()
    fig = plot_pie_chart(labels, sizes)
    st.pyplot(fig)
